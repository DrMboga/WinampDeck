...
**Update (2026-09-22):** first real-hardware check passed. On the actual Pi 3B with the AOIDE Digi Pro clone, running Raspberry Pi OS 64-bit Lite with `/boot/firmware/config.txt` set to `dtparam=audio=off` plus `dtoverlay=hifiberry-digi-pro`, `aplay -l` lists the card correctly (`card 0: sndrpihifiberry`) and `speaker-test -D hw:0,0 -c 2 -t sine` produced audible sound on both channels. This confirms the overlay and the physical DAC/output path work on the target hardware. It does **not** yet validate the `dmix`-sharing behavior this ADR is actually about — go-librespot and mpv as simultaneous `dmix` clients, one paused, switching repeatedly — which is still the open item and remains provisional until Phase 4 ([delivery-plan.md](../delivery-plan.md)) exercises it.
