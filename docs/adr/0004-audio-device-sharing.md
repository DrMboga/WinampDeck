...
**Update (2026-09-25, Phase 4 — both Engines sharing the HAT):** the open item above is closed. go-librespot and mpv ran side by side on `plug:dmixer` on the real board. Both were audible and mixed at the same time with no `EBUSY`, and a scripted soak ([pi-setup.md](../pi-setup.md#phase-4--both-engines-sharing-the-hat)) switched Source 90 times in a row: 10 cycles at 5s per Source, 30 at 1s, 50 at 0.3s. It switched the same way this ADR decided: go-librespot through `/player/pause`/`/player/resume`, mpv through `aid no`/`aid auto`. Every command succeeded, the hardware PCM showed `RUNNING` after every switch, neither Engine logged a busy/XRUN/underrun error, `get_throttled` stayed `0x0`, and the switches were audibly smooth with no clicks and no delay bringing Radio back. **The decision stands as written.**

Three things surfaced along the way that don't change the decision but do constrain the controller:

- **`aid no` persists into the next `loadfile`.** Loading a Station while `aid` is `no` makes mpv log `No video or audio streams selected`, end the file immediately and drop back to idle (`idle-active: true`); the `loadfile` reply is still `"error":"success"`. So the controller can't tune a Station while Radio is muted just by calling `loadfile`. It must select the audio track for that load (for example `aid auto` first, or a per-file `aid=auto` option on `loadfile`) and only then mute if Radio isn't the Source.
- **A muted live stream doesn't survive a long mute.** While muted with `aid no`, mpv stops pulling from the stream: after short mutes Radio resumed exactly where it was muted rather than at the live position. After a mute of several minutes, the station's server dropped the idle connection (`tls: IO error: End of file`, `https: Stream ends prematurely`). `aid auto` then brought back nothing: the PCM stayed `closed`, `demuxer-cache-state` reported `eof: true` with 0 bytes, and mpv didn't reconnect by itself. It also still reported `idle-active: false`, so the controller can't use `idle-active` to detect this. **Consequence for `RadioClient`:** switching Source to Radio can't rely on `aid auto` alone. It needs to re-`loadfile` the current Station, at least after a mute longer than some threshold, and arguably always, since a live station resuming minutes behind live is wrong anyway. Re-tuning costs the stream's connect time (a few seconds was observed on first load). Not yet measured: how long a mute this Station (or others) survives; it dropped somewhere under ~10 minutes here.
- **`bcm2835-i2s: FIFO clear timed out: no PCM clock` kernel warnings** appeared in `dmesg`, only at moments when the last client released the device while it was in an odd state: an Engine being killed mid-play, and a soak run where mpv had no Station loaded. Not every full release triggers it (a clean pause-everything later produced none). The Digi+ Pro is the I²S clock provider, so once its clock stops the Pi's I²S driver can't flush its FIFO. It's a known, commonly reported warning with HiFiBerry clock-provider boards. Nothing audible accompanied it, and playback afterwards was normal. Treated as harmless; worth re-checking if a click or failed start is ever heard when playback begins from full silence.
